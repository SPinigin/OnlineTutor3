...
namespace OnlineTutor3.Application.Services
{
    ...
    {
        private readonly ISpellingTestRepository _testRepository;
        private readonly IAssignmentRepository _assignmentRepository;
        private readonly ITeacherService _teacherService;
        private readonly ILogger<SpellingTestService> _logger;

        public SpellingTestService(
            ISpellingTestRepository testRepository,
        ...
        public async Task<SpellingTest?> GetByIdAsync(int id)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении теста по орфографии по ID: {TestId}", id);
                throw;
            }
        }
        ...
        public async Task<List<SpellingTest>> GetByTeacherIdAsync(string teacherId)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении тестов по орфографии учителя: {TeacherId}", teacherId);
                throw;
            }
        }
        ...
        public async Task<List<SpellingTest>> GetByAssignmentIdAsync(int assignmentId)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении тестов по орфографии по заданию: {AssignmentId}", assignmentId);
                throw;
            }
        }
        ...
        public async Task<List<SpellingTest>> GetActiveByTeacherIdAsync(string teacherId)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении активных тестов по орфографии учителя: {TeacherId}", teacherId);
                throw;
            }
        }

        public async Task<int> CreateAsync(SpellingTest test)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Ошибка при создании теста по орфографии: TeacherId={TeacherId}, AssignmentId={AssignmentId}", test.TeacherId, test.AssignmentId);
                throw;
            }
        }

        public async Task<int> UpdateAsync(SpellingTest test)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Ошибка при обновлении теста по орфографии: {TestId}", test.Id);
                throw;
            }
        }
        ...
        public async Task<int> DeleteAsync(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при удалении теста по орфографии: {TestId}", id);
                throw;
            }
        }
        ...
        public async Task<bool> ExistsAsync(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при проверке существования теста по орфографии: {TestId}", id);
                throw;
            }
        }
