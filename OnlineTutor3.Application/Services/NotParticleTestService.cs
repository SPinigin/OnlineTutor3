...
namespace OnlineTutor3.Application.Services
{
    ...
    /// </summary>
    public class NotParticleTestService : INotParticleTestService
    {
        private readonly INotParticleTestRepository _testRepository;
        private readonly IAssignmentRepository _assignmentRepository;
        private readonly ITeacherService _teacherService;
        private readonly ILogger<NotParticleTestService> _logger;

        public NotParticleTestService(
            INotParticleTestRepository testRepository,
            IAssignmentRepository assignmentRepository,
            ITeacherService teacherService,
            ILogger<NotParticleTestService> logger)
        {
        ...
        public async Task<NotParticleTest?> GetByIdAsync(int id)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении теста на правописание частицы \"не\" по ID: {TestId}", id);
                throw;
            }
        }
        ...
        public async Task<List<NotParticleTest>> GetByTeacherIdAsync(string teacherId)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении тестов на правописание частицы \"не\" учителя: {TeacherId}", teacherId);
                throw;
            }
        }
        ...
        public async Task<List<NotParticleTest>> GetByAssignmentIdAsync(int assignmentId)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении тестов на правописание частицы \"не\" по заданию: {AssignmentId}", assignmentId);
                throw;
            }
        }
        ...
        public async Task<List<NotParticleTest>> GetActiveByTeacherIdAsync(string teacherId)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при получении активных тестов на правописание частицы \"не\" учителя: {TeacherId}", teacherId);
                throw;
            }
        }

        public async Task<int> CreateAsync(NotParticleTest test)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Ошибка при создании теста на правописание частицы \"не\": TeacherId={TeacherId}, AssignmentId={AssignmentId}", test.TeacherId, test.AssignmentId);
                throw;
            }
        }

        public async Task<int> UpdateAsync(NotParticleTest test)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Ошибка при обновлении теста на правописание частицы \"не\": {TestId}", test.Id);
                throw;
            }
        }
        ...
        public async Task<int> DeleteAsync(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при удалении теста на правописание частицы \"не\": {TestId}", id);
                throw;
            }
        }
        ...
        public async Task<bool> ExistsAsync(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка при проверке существования теста на правописание частицы \"не\": {TestId}", id);
                throw;
            }
        }
