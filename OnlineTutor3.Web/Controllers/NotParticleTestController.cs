...
namespace OnlineTutor3.Web.Controllers
{
    ...
    public class NotParticleTestController : Controller
    {
        ...
        private readonly OnlineTutor3.Web.Services.NotParticleQuestionImportService _importService;
        private readonly UserManager<ApplicationUser> _userManager;
        private readonly ILogger<NotParticleTestController> _logger;

        public NotParticleTestController(
            INotParticleTestService testService,
            IAssignmentService assignmentService,
            ISubjectService subjectService,
            INotParticleQuestionRepository questionRepository,
            OnlineTutor3.Web.Services.NotParticleQuestionImportService importService,
            UserManager<ApplicationUser> userManager,
            ILogger<NotParticleTestController> logger)
        {
        ...
        // GET: NotParticleTest/Create?assignmentId=5
        public async Task<IActionResult> Create(int assignmentId)
        {
            try
            {
                ...
                {
                    TempData["ErrorMessage"] = "Тесты на правописание частицы \"не\" можно создавать только для предмета 'Русский язык'.";
                    return RedirectToAction("Index", "Assignment");
                }

                ViewBag.Assignment = assignment;
                var model = new CreateNotParticleTestViewModel
                {
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Create (GET) для AssignmentId: {AssignmentId}", assignmentId);
                TempData["ErrorMessage"] = "Произошла ошибка при загрузке формы создания теста.";
                return RedirectToAction("Index", "Assignment");
            }
        }

        // POST: NotParticleTest/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(CreateNotParticleTestViewModel model)
        {
            try
            {
                var currentUser = await _userManager.GetUserAsync(User);
                ...
                if (ModelState.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        if (assignmentToCheck != null)
                        {
                            ...
                            if (subject == null || !subject.Name.Equals("Русский язык", StringComparison.OrdinalIgnoreCase))
                            {
                                ModelState.AddModelError("AssignmentId", "Тесты на правописание частицы \"не\" можно создавать только для предмета 'Русский язык'.");
                            }
                        }
                    }
                }
                ...
                if (ModelState.IsValid)
                {
                    ...
                    TempData["SuccessMessage"] = $"Тест на правописание частицы \"не\" \"{test.Title}\" успешно создан! Теперь добавьте вопросы.";
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Create (POST)");
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Details для ID: {TestId}", id);
                TempData["ErrorMessage"] = "Произошла ошибка при загрузке теста.";
                return RedirectToAction("Index", "Assignment");
            }
        }

        // GET: NotParticleTest/Edit/5
        public async Task<IActionResult> Edit(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Edit (GET) для ID: {TestId}", id);
                TempData["ErrorMessage"] = "Произошла ошибка при загрузке формы редактирования.";
                return RedirectToAction("Index", "Assignment");
            }
        }

        // POST: NotParticleTest/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, CreateNotParticleTestViewModel model)
        {
            if (id <= 0)
            {
                    ...
                    if (test.AssignmentId != model.AssignmentId)
                    {
                        ...
                        else
                        {
                            ...
                            if (assignmentToCheck != null)
                            {
                                ...
                                if (subject == null || !subject.Name.Equals("Русский язык", StringComparison.OrdinalIgnoreCase))
                                {
                                    ModelState.AddModelError("AssignmentId", "Тесты на правописание частицы \"не\" можно привязывать только к заданиям по предмету 'Русский язык'.");
                                }
                            }
                        }
                    }
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Edit (POST) для ID: {TestId}", id);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Delete (GET) для ID: {TestId}", id);
                TempData["ErrorMessage"] = "Произошла ошибка при загрузке теста для удаления.";
                return RedirectToAction("Index", "Assignment");
            }
        }

        // POST: NotParticleTest/Delete/5
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/Delete (POST) для ID: {TestId}", id);
        ...
        // GET: NotParticleTest/ImportQuestions/5
        public async Task<IActionResult> ImportQuestions(int id)
        {
            try
            {
                ...
                ViewBag.Test = test;
                return View(new NotParticleQuestionImportViewModel { NotParticleTestId = id });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/ImportQuestions (GET) для ID: {TestId}", id);
                TempData["ErrorMessage"] = "Произошла ошибка при загрузке формы импорта.";
                return RedirectToAction("Details", new { id });
            }
        }

        // POST: NotParticleTest/ImportQuestions
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ImportQuestions(NotParticleQuestionImportViewModel model)
        {
            try
            {
                var currentUser = await _userManager.GetUserAsync(User);
                ...
                var sessionKey = $"ImportQuestions_{model.NotParticleTestId}_{DateTime.Now.Ticks}";
                var importData = new
                {
                    TestId = model.NotParticleTestId,
                    Questions = questions,
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/ImportQuestions (POST) для TestId: {TestId}", model.NotParticleTestId);
        ...
        // GET: NotParticleTest/DownloadTemplate
        public async Task<IActionResult> DownloadTemplate()
        {
            try
            {
                ...
                return File(templateBytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    "Шаблон_импорта_вопросов_частица_не.xlsx");
            }
            catch (Exception ex)
            {
        ...
        // GET: NotParticleTest/PreviewQuestions
        public async Task<IActionResult> PreviewQuestions()
        {
            try
            {
                ...
                {
                    var question = System.Text.Json.JsonSerializer.Deserialize<ImportNotParticleQuestionRow>(questionElement.GetRawText());
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/PreviewQuestions");
                TempData["ErrorMessage"] = "Ошибка обработки данных.";
                return RedirectToAction("Index", "Assignment");
            }
        }

        // POST: NotParticleTest/ConfirmImport
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ConfirmImport()
        {
                ...
                foreach (var importQuestion in questions)
                {
                    try
                    {
                        var question = new NotParticleQuestion
                        {
                            ...
                            TextWithGap = importQuestion.TextWithGap ?? "",
                            CorrectAnswer = (importQuestion.CorrectAnswer ?? "").Trim().ToLower(),
                            FullText = importQuestion.FullText ?? "",
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/ConfirmImport");
                TempData["ErrorMessage"] = "Ошибка при импорте вопросов.";
                return RedirectToAction("Index", "Assignment");
            }
        }

        // POST: NotParticleTest/DeleteAllQuestions/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> DeleteAllQuestions(int id)
        {
            ...
            {
                _logger.LogError(ex, "Ошибка в NotParticleTest/DeleteAllQuestions для ID: {TestId}", id);
